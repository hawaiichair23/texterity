...
async function initPixi() {
    ...
    textContainer.on('pointerdown', (event) => {
        isDragging = true;
        const position = event.data.global;
        const bounds = textContainer.getBounds();
        dragOffset.x = position.x - bounds.x;
        dragOffset.y = position.y - bounds.y;
        textContainer.alpha = 0.7; // Visual feedback while dragging
    });
    ...
    textContainer.on('pointerupoutside', () => {
        if (isDragging) {
            // Save the custom position
            const bounds = textContainer.getBounds();
            customPosition = {
    ...
    textContainer.on('pointermove', (event) => {
        if (isDragging) {
            const position = event.data.global;
            const bounds = textContainer.getBounds();

            // Move all children by the delta
